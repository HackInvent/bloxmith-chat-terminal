...
class ChatTerminalBlock(BlockDefinition):
    ...
    def _render_modal_config_fields(self, node: dict[str, Any]) -> str:
        ...
        return (
            ...
            '<p class="chat-terminal-secret-note">The key is used on the server only and is never returned in the outputs.</p></div>'
            ...
            f'<textarea data-block-config-field="speech_prompt" rows="3" spellcheck="false" placeholder="Optional context to guide the transcription...">{escape(speech["speech_prompt"])}</textarea></div>'
            '</div>'
        )
    ...
    def _render_modal_attributes(self, *, node: dict[str, Any], payload: dict[str, Any]) -> str:
        ...
        return (
            ...
            '</div>'
            '<div class="inspector-actions"><button class="solid-btn" data-block-apply type="button" disabled>Apply</button></div>'
            '</section>'
            '<section class="ports-editor-section">'
